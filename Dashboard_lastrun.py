...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='C:\\Users\\William Chen\\Desktop\\ResearchStudyDashboard\\Dashboard_lastrun.py',
...
RegulationRating_Text = visual.TextStim(win=win, name='RegulationRating_Text',
    text='I successfully implemented the reappraisal strategy in the last block.',
    font='Open Sans',
...
RegulationRating_Response = visual.Slider(win=win, name='RegulationRating_Response',
    startValue=None, size=(1.4, 0.15), pos=(0, -0.14), units=win.units,
...
ConditionText = visual.TextStim(win=win, name='ConditionText',
    text='What robot condition is being used for this block?',
    font='Open Sans',
...
GetRead_Cross = visual.ShapeStim(
    ...
    lineWidth=1.0,     colorSpace='rgb',  lineColor='white', fillColor=[0.6549, 0.6549, 0.6549],
...
Trial_RatingResponse = visual.Slider(win=win, name='Trial_RatingResponse',
    startValue=None, size=(1.5, 0.15), pos=(0, -0.14), units=win.units,
    labels=("Not at all", "Slightly", "Moderately", "Very", "Extremely"), ticks=(1, 2, 3, 4, 5), granularity=1.0,
    style='rating', styleTweaks=(), opacity=None,
    labelColor=[-1.0000, -1.0000, -1.0000], markerColor=[1.0000, 1.0000, -1.0000], lineColor=[-1.0000, -1.0000, -1.0000], colorSpace='rgb',
...
ConditionDone.reset()
# keep track of which components have finished
CondComponents = [ConditionTextBox, ConditionDone, ConditionText]
...
while continueRoutine:
    ...
    # if ConditionText is starting this frame...
    if ConditionText.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
        # keep track of start time/frame for later
        ConditionText.frameNStart = frameN  # exact frame index
        ConditionText.tStart = t  # local t and not account for scr refresh
        ConditionText.tStartRefresh = tThisFlipGlobal  # on global time
        win.timeOnFlip(ConditionText, 'tStartRefresh')  # time at next scr refresh
        # add timestamp to datafile
        thisExp.timestampOnFlip(win, 'ConditionText.started')
        # update status
        ConditionText.status = STARTED
        ConditionText.setAutoDraw(True)
    
    # if ConditionText is active this frame...
    if ConditionText.status == STARTED:
        # update params
        pass
...
for thisNSB1 in NSB1:
    ...
    continueRoutine = True
    # update component parameters for each repeat
    Trial_NSB.setText(NSBs)
    Trial_Cue.setText(Cue)
    ...
    NSB1.addData('Trial_RatingResponse.history', Trial_RatingResponse.getHistory())
...
for thisNeuSta1 in NeuSta1:
    ...
    continueRoutine = True
    # update component parameters for each repeat
    NeutralStatement_Text.setText(NeutralStatement)
    # keep track of which components have finished
...
for thisNSB2 in NSB2:
    ...
    NSB2.addData('Trial_RatingResponse.history', Trial_RatingResponse.getHistory())
...
for thisNSB3 in NSB3:
    ...
    NSB3.addData('Trial_RatingResponse.history', Trial_RatingResponse.getHistory())
...
for thisNSB4 in NSB4:
    ...
    NSB4.addData('Trial_RatingResponse.history', Trial_RatingResponse.getHistory())
...
for thisNSB5 in NSB5:
    ...
    NSB5.addData('Trial_RatingResponse.history', Trial_RatingResponse.getHistory())
...
for thisNSB6 in NSB6:
    ...
    NSB6.addData('Trial_RatingResponse.history', Trial_RatingResponse.getHistory())
